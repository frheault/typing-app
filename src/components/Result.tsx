...
export default function Result({
  ...
  const certificateRef = useRef<HTMLDivElement>(null); // Keep ref for potential future use or if other parts rely on it.

  const handleDownloadPdf = async () => {
    if (!/^[A-Z]{4}\d{4}$/.test(cipCode)) {
      console.error("CIP is invalid.");
      alert("Please enter a valid CIP code (e.g., ABCD1234).");
      return;
    }
    ...
    try {
      ...
      const tableData = [
        ...
        ["CIP", cipCode],
        ["Date of Test Completion", displayedTimestamp],
        ["Typing Speed (CPM)", cpm.toString()],
        ["Accuracy", `${accuracy}%`],
        ["Duration", `${duration} seconds`],
        ["Errors", totalErrors.toString()],
        ["Test Integrity Hash (SHA-256)", textHash],
      ];
      ...
      autoTable(pdf, {
        ...
        theme: "grid",
        styles: {
          font: "helvetica", // Ensure word-searchable font
          cellPadding: 3,
          fontSize: 10,
        },
        headStyles: {
      ...
      alert(
        "Failed to generate PDF. Please try again or check the console for more details if the issue persists."
      );
    }
  };
